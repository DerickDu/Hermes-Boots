const domain = "https://charming-storm-362222.uk.r.appspot.com";
// const domain = "";
// my expection: credentials = {username: string, password, string}
...
export const login = (credential, asHost) => {
  // backstick wrap template string
  const loginUrl = `${domain}/authenticate/${asHost ? "host" : "guest"}`; // determine if the user is host, if not, guest
  return fetch(loginUrl, {
    method: "POST",
    headers: {
      "Content-Type": "application/json", // this is a regulation, in order to tell the backend this is a json file to be deserialized.
    },
    body: JSON.stringify(credential), // flatten the JSON
  }).then((response) => {
    // if the fetch success, then do the function inside the then
    if (response.status !== 200) {
      throw Error("Fail to log in"); // browser provide this class 'Error'
    }
...
export const getReservations = () => {
  ...
  return fetch(listReservationsUrl, {
    // get request do not need to write method = "GET", because that is default
    headers: {
    ...
    return response.json(); // depends on the backend design, if backend return json file then write .json, otherwise, .string or sth else.
  });
};
...
export const searchStays = (query) => {
  ...
  searchStaysUrl.searchParams.append("guest_number", query.guest_number); // quary string: http://localhost:3000/?a=1&b=2
